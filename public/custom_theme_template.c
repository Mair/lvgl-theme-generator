...
static void basic_init(void)
{
    ...
    lv_style_set_radius(&styles->bg, LV_STATE_DEFAULT, {{borderRadius}});
    ...
    lv_style_set_radius(&styles->btn, LV_STATE_DEFAULT,  {{borderRadius}});
 ...
 */
lv_theme_t * lv_theme_custom_init()
{
   lv_color_t color_primary = lv_color_hex({{colorPrimary}});
   
    lv_color_t color_secondary = lv_color_hex({{secondaryColor}});
    uint32_t flags = LV_THEME_MATERIAL_FLAG_LIGHT;
    const lv_font_t* font_small = &lv_font_montserrat_12;
    const lv_font_t* font_normal = &lv_font_montserrat_16;
    const lv_font_t* font_subtitle = &lv_font_montserrat_20;
